import React, { useState, useEffect } from "react";
import { Icon, Menu, Table } from "semantic-ui-react";
import CitiesOfTurkeyService from "../services/citiesOfTurkeyService";

export default function CitiesOfTurkeyList() {
  const [citiesOfTurkeys, setCitiesOfTurkeys] = useState([]);

  useEffect(() => {
    let citiesOfTurkeyService = new CitiesOfTurkeyService();
    citiesOfTurkeyService
      .getCitiesOfTurkey()
      .then((result) => setCitiesOfTurkeys(result.data.data));
  },[]);

  return (
    <div>
    <Table celled>
      <Table.Header>
        <Table.Row>
          <Table.HeaderCell>Şehir No</Table.HeaderCell>
          <Table.HeaderCell>Şehir Adı</Table.HeaderCell>
          <Table.HeaderCell>Plaka Kodu</Table.HeaderCell>
        </Table.Row>
      </Table.Header>

      <Table.Body>
        {citiesOfTurkeys.map((citiesOfTurkey) => (
          <Table.Row key={citiesOfTurkey.id}>
            <Table.Cell>{citiesOfTurkey.id}</Table.Cell>
            <Table.Cell>{citiesOfTurkey.citiesOfTurkeyName}</Table.Cell>
            <Table.Cell>{citiesOfTurkey.citiesOfTurkeyPlateId}</Table.Cell>
          </Table.Row>
        ))}
      </Table.Body>

      <Table.Footer>
        <Table.Row>
          <Table.HeaderCell colSpan="3">
            <Menu floated="right" pagination>
              <Menu.Item as="a" icon>
                <Icon name="chevron left" />
              </Menu.Item>
              <Menu.Item as="a">1</Menu.Item>
              <Menu.Item as="a">2</Menu.Item>
              <Menu.Item as="a">3</Menu.Item>
              <Menu.Item as="a">4</Menu.Item>
              <Menu.Item as="a" icon>
                <Icon name="chevron right" />
              </Menu.Item>
            </Menu>
          </Table.HeaderCell>
        </Table.Row>
      </Table.Footer>
    </Table>
  </div>
);
}