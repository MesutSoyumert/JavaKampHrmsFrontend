import React, { useState, useEffect } from "react";
import { Icon, Menu, Table } from "semantic-ui-react";
import BaseUserService from "../services/baseUserService";

export default function BaseUserList() {
  const [baseUsers, setBaseUsers] = useState([]);

  useEffect(() => {
    let baseUserService = new BaseUserService();
    baseUserService
      .getBaseUsers()
      .then((result) => setBaseUsers(result.data.data));
  },[]);

  return (
    <div>
    <Table celled>
      <Table.Header>
        <Table.Row>
          <Table.HeaderCell>Kullanıcı No</Table.HeaderCell>
          <Table.HeaderCell>E-posta Adresi</Table.HeaderCell>
          <Table.HeaderCell>Parola</Table.HeaderCell>
          <Table.HeaderCell>Aktif mi</Table.HeaderCell>
        </Table.Row>
      </Table.Header>

      <Table.Body>
        {baseUsers.map((baseUser) => (
          <Table.Row key={baseUser.id}>
            <Table.Cell>{baseUser.id}</Table.Cell>
            <Table.Cell>{baseUser.emailAddress}</Table.Cell>
            <Table.Cell>{baseUser.password}</Table.Cell>
            {baseUser.status ? (
              <Table.Cell>Aktif</Table.Cell>
            ) : (
              <Table.Cell>Pasif</Table.Cell>
            )}
          </Table.Row>
        ))}
      </Table.Body>

      <Table.Footer>
        <Table.Row>
          <Table.HeaderCell colSpan="3">
            <Menu floated="right" pagination>
              <Menu.Item as="a" icon>
                <Icon name="chevron left" />
              </Menu.Item>
              <Menu.Item as="a">1</Menu.Item>
              <Menu.Item as="a">2</Menu.Item>
              <Menu.Item as="a">3</Menu.Item>
              <Menu.Item as="a">4</Menu.Item>
              <Menu.Item as="a" icon>
                <Icon name="chevron right" />
              </Menu.Item>
            </Menu>
          </Table.HeaderCell>
        </Table.Row>
      </Table.Footer>
    </Table>
  </div>
);
}